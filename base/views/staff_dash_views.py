...
def unverifiedforms(request):
    ...
    user_info = [(form.user.first_name, form.user.last_name) for form in unverified_forms]
    
    context = {'unverified_forms': unverified_forms, 'user_info': user_info}
...
def suspended_users(request):
    search_query = request.GET.get('search_query')
    patients = User.objects.filter(is_staff=False, is_active=False)
    ...
    context = {'patients': patients}
    return render(request, 'base/staff-section/suspended_users.html', context)
